...
distribution_plot <- function(datas,rep_patientID_unique, my.cols, cancertype, datatype){
  ...
  plots<- ggplot(data = datas ,aes(Samples,Genecount, color = as.factor(Match_id))) + 
    ...
    theme_light()+
    theme(legend.text = element_text(size = 14),axis.text.x = element_blank(), axis.title=element_text(size=14),
          legend.title=element_text(size=16), plot.title = element_text(size=20)) + 
